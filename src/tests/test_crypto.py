...
class TestCrypto(unittest.TestCase):
    ...
    def test_unpad_malformed_payload(self):
        ...
        malformed_data = (
            b"\x00\x00\x00\x10hello world"  # Length doesn't match actual data
        )
        with self.assertRaises(ValueError):
    ...
    def test_generate_cycle(self):
        ...
        with self.assertRaises(ValueError):
            crypto.generate_cycle(
                TEST_USER_LIST, "Alice", "Bob", 5
            )  # Cycle length too short

        with self.assertRaises(ValueError):
            crypto.generate_cycle(
                {"Alice": TEST_USER_LIST["Alice"]}, "Alice", "Bob", 6
            )  # Not enough users
    ...
    def test_header(self):
        ...
        for x in range(6):
            f, i, p, header = crypto.decode_header(
                header, base64.b64decode(TEST_USER_LIST[cycle[x][0]]["prikey"])
            )
            self.assertEqual(len(header), orig_header_len)
            self.assertEqual(f, 3 if x == 5 else 2 if x == 2 else 0)
            self.assertEqual(
                i, req_id if x == 5 else TEST_USER_LIST[cycle[x + 1][0]]["ip"]
            )
            self.assertEqual(
                p, 0 if x == 5 else TEST_USER_LIST[cycle[x + 1][0]]["port"]
            )
    ...
    def test_get_header_size(self):
        ...
        cycle_length = 6
        expected_size = (
            crypto._HEADER_STRUCT.size + crypto._ENCRYPT_HEADER_STRUCT.size
        ) * cycle_length
